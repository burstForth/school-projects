...
<html class="x-admin-sm">
  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        <shiro:hasPermission name="xmt:add"> 
        <button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon"></i>批量删除</button>
        </shiro:hasPermission>
          <shiro:hasPermission name="xmt:delete"> 
        <button class="layui-btn" onclick="x_admin_show('添加项目','/HouseCms/BackEnd/jsp/project-add.jsp')"><i class="layui-icon"></i>添加</button>
        </shiro:hasPermission>
      ...
      <table class="layui-table x-admin">
        ...
        <tbody>
          ...
          <tr>
            <td>
              <input type="checkbox" value="${p.pid}"   name="projectCheck"  class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'/>
            </td>
            <td>${p.pid}</td>
            <td><img  src="/HouseCms/${p.url}"></td>
            <td>${p.ptype}</td>
            <td>${p.pname}</td>
            <td>${fn:substring(p.pdescribe,0,40)}...</td>
           
            <td class="td-status">
              <c:if test="${p.pstatus=='2'}">
            ...
            <td class="td-manage">
              <shiro:hasPermission name="xmt:stastu">
              <a onclick="member_stop(this,${p.pstatus},${p.pid})" href="javascript:;"  title="启用">
               <c:if test="${p.pstatus==2}">
                ...
                <shiro:hasPermission name="xmt:delete">
              <a title="删除" onclick="member_del(this,'${p.pid}')" href="javascript:;">
                <i class="layui-icon">&#xe640;</i>
              </a>
              </shiro:hasPermission>
            </td>
    ...
    <script>
      ...
      function delAll () {
    	  var checkedNum = $("input[name='projectCheck']:checked").length;
  	    if (checkedNum == 0) {
        ...
        $("input[name='projectCheck']:checked").each(function(){
        	projectList.push($(this).val());
        });
	    $.ajax({
            type : "post",
            url : "/HouseCms/project/batchDelete",
            data : {projectlist : projectList.toString()},
            success : function(){
    ...
    <script type="text/javascript"> 
     $(function(){
    	...
    	$("input[name='projectCheck']").click(function() {
			var a=0;
			var box=document.getElementsByName("projectCheck");
			var b=document.getElementById("selectAll");
			$("input[name='projectCheck']").each(function(i,e) {
				if (box[i].checked) {
					a++;
				}
				if (box.length==a) {
 ...
 <script type="text/javascript">
    function chaxun(currpage,rows){
       window.location="/HouseCms/project/findallht?currpage="+currpage+"&rows="+rows;
    }  
    </script>
  </body>
