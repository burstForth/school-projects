...
<article class="page-container">
	<form action="/HouseCms/role/update" method="post" class="form form-horizontal" id="form-admin-role-add">
		<input type="hidden" value="${role.rid}" name="rid"> 
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>角色名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${role.rname}" placeholder="" id="roleName" name="rname">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<c:forEach items="${list}" var="m">
				...
				<c:if test="${m.type=='menu1'}">
					  <input type="checkbox" 
						    <c:forEach items="${flist}" var="fm">
									<c:if test="${m.mid==fm.mid}">
									checked="checked" 
									</c:if>
				     		</c:forEach>
				        value="${m.mid}"  name="mid" id="user-Character-0">${m.mname}
	    		</c:if>
					</dt>
					<dd>
					<c:forEach items="${listmenu}" var="ml">
						<dl class="cl permission-list2">
							...
							<c:if test="${m.mid==ml.parene_id && ml.type=='menu'}">
								<label class="">
									<input type="checkbox" 
										<c:forEach items="${flistmenu}" var="fml">
											<c:if test="${ml.mid==fml.mid}">
											checked="checked" 
											</c:if>
										</c:forEach>
									value="${ml.mid}" name="mid" id="user-Character-0-0">${ml.mname}
								</label>
							 </c:if>
							</dt>
							...
							<c:if test="${m.mid==ml.parene_id && ml.type=='menu' && ml.mid==lb.parene_id && lb.type=='button'}">
								<label class="">
									<input type="checkbox"
										 <c:forEach items="${flistbutton}" var="flb">
											  <c:if test="${lb.mid==flb.mid}">
											  checked="checked" 
											  </c:if>
										  </c:forEach>
									   value="${lb.mid}" name="mid" id="user-Character-0-0-0">${lb.mname}
									</label>
								</c:if>
						  </c:forEach>
							</dd>
